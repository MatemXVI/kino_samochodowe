...
<?php
	...
	if(isset($_SESSION['admin'])){		
		...
		$row = $result->fetch_array();
 		...
 		if(!file_exists($path_film.$katalog."/".$row['nazwa_plakatu'])){
			if ($mysqli->query("UPDATE film SET nazwa_plakatu = NULL WHERE IDFilmu ='$id'")) {
...
<?php		}																?>	
			...
			<nav class='administrator'>
				<a href='edytuj_film.php?id_filmu=<?php echo $id ?>'>Wróć do menu</a>
			</nav>			
        </header>
		<section>
...
<?php 				if($row['nazwa_plakatu'] != NULL){ ?>
						<div class="poster">
							<img src="<?php echo $path_film.$katalog."/".$row['nazwa_plakatu'] ?>" width="336" height="500">
						</div>
						<div>
							<div class = "plik_formularz">
								<div class="nazwa_pliku">
									...
									<div class="przycisk_zmiany">
										...
										<button class="przycisk" type="submit" name="usun_plakat" /><b>Usuń</b></button></label><br>
									</div>
...
<?php 				}else{
							...
							<input type="hidden" name="id_filmu" value="<?php echo $id ?>" />
