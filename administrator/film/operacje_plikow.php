...
<?php
	...
	if(isset($_SESSION['admin'])){		
		...
		if(isset($_POST['zmien_nazwe_plakatu'])){
			$result = $mysqli->query("SELECT nazwa_plakatu FROM film WHERE IDFilmu = ".$id);
			$row = $result->fetch_array();
			if(isset($_POST['nazwa_plakatu'])){
				$nazwa_plakatu=$_POST['nazwa_plakatu'];
				if($nazwa_plakatu==$row['nazwa_plakatu']){
					header("Location: obsluga_plikow.php?id_filmu=$id&katalog=$katalog");	
					$_SESSION['plakaty']="<span>Dane które edytujesz są takie same!</span>";
					exit;
				}	
				$result = $mysqli->query("SELECT nazwa_zdjecia FROM film_zdjecia WHERE IDFilmu = $id AND nazwa_zdjecia= '$nazwa_plakatu'");
				$row = $result->fetch_array();
				...
				$result = $mysqli->query("SELECT nazwa_plakatu FROM film WHERE IDFilmu = $id");
				$row = $result->fetch_array();
				$oldfile=$path_film.$katalog."/".$row[0];
				$newfile=$path_film.$katalog."/".$nazwa_plakatu;
				if(!rename($oldfile, $newfile)){
					$_SESSION['plakaty']='Nie zmieniono nazwy pliku!';
					header("Location: obsluga_plikow.php?id_filmu=$id&katalog=$katalog");
					exit;
				}else{
					$result = $mysqli->query("UPDATE `film` SET `nazwa_plakatu` = '$nazwa_plakatu' WHERE IDFilmu = $id");
			...
			if(isset ($_FILES['nazwa_plakatu']['name'])){	
				if(($_FILES['nazwa_plakatu']['error']) <= 0){	
					if (is_uploaded_file($_FILES['nazwa_plakatu']['tmp_name'])) {											
						...
						$result = $mysqli->query("SELECT nazwa_plakatu FROM film WHERE IDFilmu = $id AND nazwa_plakatu= '$nazwa_plakatu'");
						$row = $result->fetch_array();
						if ($result->num_rows>0) {
							$_SESSION['plakaty']='Plik już istnieje!';
							header("Location: obsluga_plikow.php?id_filmu=$id&katalog=$katalog");		
						...
						if(!move_uploaded_file($_FILES['nazwa_plakatu']['tmp_name'], $path_film.$katalog."/".$_FILES['nazwa_plakatu']['name'])){
		...
		//usunięcie plakatu
			...
			if(isset($_POST['nazwa_plakatu'])){
				$nazwa_plakatu=$_POST['nazwa_plakatu'];
				$result = $mysqli->query("UPDATE `film` SET `nazwa_plakatu` = NULL WHERE IDFilmu = $id");
				if($mysqli->connect_errno){
					$_SESSION['plakaty']="<span style='color:red'>Error: " . $result . "<br>" . $mysqli->errno."</span>";
					header("Location: obsluga_plikow.php?id_filmu=$id&katalog=$katalog");
					exit;
				}
				if(unlink($path_film.$katalog."/".$nazwa_plakatu)){
		...
		if(isset($_POST['zmien_nazwe_zdjecia'])){
			$result = $mysqli->query("SELECT nazwa_zdjecia FROM film_zdjecia WHERE IDFilmu = ".$id);
			$row = $result->fetch_array();
			if(isset($_POST['nazwa_zdjecia'])){
				$nazwa_zdjecia=$_POST['nazwa_zdjecia'];
				if($nazwa_zdjecia==$row['nazwa_zdjecia']){
					header("Location: obsluga_plikow.php?id_filmu=$id&katalog=$katalog");	
					$_SESSION['obsluga_plikow']="<span>Dane które edytujesz są takie same!</span>";
					exit;
				}
				$result = $mysqli->query("SELECT nazwa_plakatu FROM film WHERE IDFilmu = $id AND nazwa_plakatu= '$nazwa_zdjecia'");
				$row = $result->fetch_array();
				if ($result->num_rows>0) {
				...
				$row = $result->fetch_array();
				$oldfile=$path_film.$katalog."/".$row[0];
				$newfile=$path_film.$katalog."/".$nazwa_zdjecia;
				...
				if(!rename($oldfile, $newfile)){
					...
					$result = $mysqli->query("UPDATE `film_zdjecia` SET `nazwa_zdjecia` = '$nazwa_zdjecia' WHERE IDFilmu = $id");
					if($mysqli->connect_errno){
			...
			if(isset ($_FILES['nazwa_zdjecia']['name'])){	
				if(($_FILES['nazwa_zdjecia']['error']) <= 0){	
					if (is_uploaded_file($_FILES['nazwa_zdjecia']['tmp_name'])) {
						...
						$result = $mysqli->query("SELECT nazwa_zdjecia FROM film_zdjecia WHERE IDFilmu = $id AND nazwa_zdjecia= '$nazwa_zdjecia'");
						if ($result->num_rows>0) {
							$_SESSION['zdjecia']='Plik już istnieje!';
							header("Location: obsluga_plikow.php?id_filmu=$id&katalog=$katalog");		
							exit;
						...
						$result = $mysqli->query("INSERT INTO film_zdjecia VALUE ('$nazwa_zdjecia', $id)");
						if($mysqli->connect_errno){
							$_SESSION['zdjecia']="<span style='color:red'>Error: " . $result . "<br>" . $mysqli->errno."</span>";
							header("Location: obsluga_plikow.php?id_filmu=$id&katalog=$katalog");
							exit;
						}
						if(!move_uploaded_file($_FILES['nazwa_zdjecia']['tmp_name'], $path_film.$katalog."/".$_FILES['nazwa_zdjecia']['name'])){
		...
		//usunięcie zdjecia
			...
			if(isset($_POST['nazwa_zdjecia'])){
				$nazwa_zdjecia=$_POST['nazwa_zdjecia'];
				$result = $mysqli->query("DELETE FROM `film_zdjecia` WHERE IDFilmu = $id AND nazwa_zdjecia = '$nazwa_zdjecia'");
				if($mysqli->connect_errno){
					$_SESSION['zdjecia']="<span style='color:red'>Error: " . $result . "<br>" . $mysqli->errno."</span>";
					header("Location: obsluga_plikow.php?id_filmu=$id&katalog=$katalog");
					exit;
				}
				if(unlink($path_film.$katalog."/".$nazwa_zdjecia)){
