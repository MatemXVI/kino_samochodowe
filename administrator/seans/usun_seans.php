...
<?php
	...
	if(isset($_SESSION['admin'])){	
		if(isset($_GET['usun_id'])){
			$id_seansu = $_GET['usun_id'];							
			if ($result = $mysqli->query("SELECT nazwa FROM `seans` WHERE IDSeansu ='$id_seansu'")) {
				$row = $result->fetch_array();
				$katalog=filter_filename($row['nazwa']);
				if($mysqli->query("DELETE FROM seans WHERE IDSeansu ='$id_seansu'")){	
					deleteDirectory($path_seans.$katalog);
				}				
				$_SESSION['komunikat']="<span style='color:green'>Seans został usunięty z bazy.</span>";
...
<?php		}																?>	
		...
		<section>
			<?php
				if(isset($_SESSION['admin'])){ 
					echo "Czy chcesz usunąć seans?"
					?>
					<div id="user">
						<div class="ustawienia_user">
							<a href="?usun_id=<?php echo $_GET['id_seansu'] ?>"><b>Tak</b></a>				
						</div>
						<div class="ustawienia_user">
